...
const rhythmShapes = {
  simple1: [
    { dur: 4, label: "q" }, { dur: 4, label: "q" }, { dur: 4, label: "q" }, { dur: 4, label: "q" }
  ],
  simple2: [
    { dur: 8, label: "h" }, { dur: 4, label: "q" }, { dur: 4, label: "q" }
  ],
  eighth1: [
    { dur: 4, label: "q" }, { dur: 2, label: "e" }, { dur: 2, label: "e" }, { dur: 4, label: "q" }, { dur: 4, label: "q" }
  ],
  eighth2: [
    { dur: 2, label: "e" }, { dur: 2, label: "e" }, { dur: 4, label: "q" }, { dur: 2, label: "e" }, { dur: 2, label: "e" }, { dur: 4, label: "q" }
  ],
  eighth3: [
    { dur: 4, label: "q" }, { dur: 4, label: "q" }, { dur: 2, label: "e" }, { dur: 2, label: "e" }, { dur: 4, label: "q" }
  ]
};
...
function init() {
  updateScore();
  setStatus("NEW を押して、演奏された旋律を3つの譜例から選んでください。");
}
...
function getLengthBars() {
  return Number(document.querySelector('input[name="length"]:checked').value) || 1;
}
...
function newQuestion() {
  ...
  const lengthBars = getLengthBars();
  const rhythmMode = getRhythmMode();
  const level = getLevel();
  const rhythm = chooseRhythm(lengthBars, rhythmMode);
  const correct = generateMelody(key, rhythm, level);
  const distractors = generateDistractors(correct, key, rhythm, level, 2);
  ...
  choiceKeyHeader.textContent = `KEY: ${key.label} / ${lengthBars} BAR${lengthBars > 1 ? "S" : ""}`;

  renderChoices();
  setStatus(`${key.label} / ${lengthBars}小節 / ${levelLabel(level)}。`);
  playCurrentQuestion();
}

function chooseRhythm(lengthBars, rhythmMode) {
  const shapeNames = rhythmMode === "simple"
    ? ["simple1", "simple2"]
    : ["simple1", "eighth1", "eighth2", "eighth3"];

  let events = [];
  for (let bar = 0; bar < lengthBars; bar += 1) {
    const shape = rhythmShapes[randomItem(shapeNames)].map((item) => ({ ...item }));
    events = events.concat(shape);
  }
  return events;
}

function generateMelody(key, rhythm, level) {
  const maxStep = level === "step" ? 1 : level === "thirds" ? 2 : 4;
  const degreeRange = key.mode === "minor" ? [0, 1, 2, 3, 4, 5, 6, 7] : [0, 1, 2, 3, 4, 5, 6, 7];
  const notes = [];
  let currentDegree = randomItem([0, 2, 4]);
  const contour = [];

  rhythm.forEach((event, index) => {
    if (index === 0) {
      currentDegree = randomItem([0, 2, 4]);
    } else {
      const possibleMoves = level === "step"
        ? [-1, 0, 1]
        : level === "thirds"
          ? [-2, -1, 0, 1, 2]
          : [-4, -3, -2, -1, 0, 1, 2, 3, 4];

      let move = randomItem(possibleMoves);
      let next = currentDegree + move;
      let guard = 0;
            while ((next < 0 || next > 7) && guard < 20) {
        move = randomItem(possibleMoves);
        next = currentDegree + move;
        guard += 1;
      }
      next = Math.max(0, Math.min(7, next));
      contour.push(move > 0 ? "↑" : move < 0 ? "↓" : "→");
      currentDegree = next;
    }

    const midi = degreeToMidi(key, currentDegree);
    notes.push({ midi, degree: currentDegree, dur: event.dur, label: event.label });
  });

  // Make final note stable when possible.
  const last = notes[notes.length - 1];
  if (last && Math.random() < 0.65) {
    const finalDegree = randomItem([0, 2, 4]);
    last.degree = finalDegree;
    last.midi = degreeToMidi(key, finalDegree);
  }

  return finalizeMelody(key, notes, contour);
}

function generateDistractors(correct, key, rhythm, level, count) {
  const out = [];
  ...
  while (out.length < count && guard < 80) {
    guard += 1;
    const candidate = mutateMelody(correct, key, level);
  ...
  while (out.length < count) {
    const candidate = generateMelody(key, rhythm, level);
    const k = melodyKey(candidate.notes);
    if (!seen.has(k)) {
      seen.add(k);
      out.push(candidate);
...
function mutateMelody(correct, key, level) {
  const notes = correct.notes.map((note) => ({ ...note }));
  const maxDelta = level === "step" ? 1 : level === "thirds" ? 2 : 3;
  const changeCount = Math.min(notes.length, randomItem([1, 2, 2]));

  const indices = shuffle(Array.from({ length: notes.length }, (_, i) => i)).slice(0, changeCount);
  indices.forEach((index) => {
    const delta = randomItem([-maxDelta, -1, 1, maxDelta].filter((value) => value !== 0));
    const nextDegree = Math.max(0, Math.min(7, notes[index].degree + delta));
    notes[index].degree = nextDegree;
    notes[index].midi = degreeToMidi(key, nextDegree);
  });

  return finalizeMelody(key, notes, contourFromNotes(notes));
}

function finalizeMelody(key, notes, contour) {
...
function melodyKey(notes) {
  return notes.map((note) => `${note.midi}:${note.dur}`).join("|");
}
...
async function playCurrentQuestion() {
  ...
  setStatus(`${currentQuestion.key.label} / ${currentQuestion.lengthBars}小節の旋律です。`);
}
...
function answer(choiceId) {
  ...
  setStatus(
    `${isCorrect ? "正解" : "不正解"} / ${currentQuestion.key.label} / ${formatResponseTime(latestResponseTimeSec)}`,
  ...
  resultLog.push({
    ...
    lengthBars: currentQuestion.lengthBars,
...
function showAnswer() {
  ...
  answerText.textContent = `正解：${currentQuestion.key.label} / ${currentQuestion.lengthBars}小節 / ${levelLabel(currentQuestion.level)}`;
  analysisText.textContent = `旋律輪郭：${currentQuestion.correct.contour.join(" ") || "単音"}`;
  renderAbc("notation", currentQuestion.correct.abc, 540);
}

function buildAbc(key, notes) {
  const body = notes.map((note) => `${midiToAbc(note.midi)}${durToAbc(note.dur)}`).join(" ");
  return `X:1
M:4/4
L:1/16
K:${key.keySig}
...
function renderHistory() {
  ...
  resultLog.slice().reverse().forEach((item) => {
    ...
    row.innerHTML = `
      ...
      <span>${item.keyLabel} / ${item.correctContour || "-"} / ${formatResponseTime(item.responseTimeSec)}</span>
...
async function exportResultsPdf() {
  ...
  try {
    ...
    for (const item of resultLog) {
      ...
      doc.setFont("helvetica", "bold");
      doc.setFontSize(10);
      doc.text(`${String(item.number).padStart(2, "0")}  ${item.keyLabel}  ${item.isCorrect ? "OK" : "NG"}`, 16, y);
      ...
      if (item.correctAbc) {
        ...
        const correctSize = fitImageSize(correctNotation, 174, 26);
      ...
      if (item.selectedAbc) {
        ...
        const selectedSize = fitImageSize(selectedNotation, 174, 26);
