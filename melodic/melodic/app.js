...
function melodicNoteToAbc(note, key) {
  // Key-aware, theory-aware melodic spelling.
  //
  // Important ABC rule:
  // - If a note belongs to the key signature, write the plain letter only.
  //   Example in Db major: F, B, E, A, D, G, C are rendered with the key signature.
  // - If a note deviates from the key signature, write the absolute accidental:
  //   =B for B natural in C minor, not ^B.
  //   ^G for G# in A minor.
  //   _E for Eb if an actual chromatic flat is needed.
...
function absoluteAccidentalFromNatural(actualPc, naturalPc) {
  // Return accidental relative to the natural letter, not relative to key signature.
  // 0 means natural, 1 sharp, -1 flat, 2 double sharp, -2 double flat.
